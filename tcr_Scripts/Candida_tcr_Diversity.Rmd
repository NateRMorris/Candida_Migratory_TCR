...
fig_15 <- ggplot(gf_new, aes(x = Mouse_New, y = value, color = Mouse_New)) +
  ...
  labs(x = "Cluster", y = "Gini Index Value", title = "Dotplot of Gini Index Values for each Mouse by Organ within Clusters", fill = "Organ") + 
    facet_wrap(~ cluster, scales = "free_x", nrow = 1) + # If I want to use this, change color = organ and rid of stuff in geom_point
    theme(axis.text.x = element_text(angle = 45, hjust = 1), plot.margin = margin(l = 25))  # Rotate x-axis labels by 45 degrees
```
...
ggsave(filename = paste0(fig_directory, "figure_15.pdf"), plot = fig_15, device = "pdf", height = 12, width = 24)

ggsave(filename = paste0(fig_directory, "figure_15.jpeg"), plot = fig_15, device = "jpeg", height = 12, width = 24)
```
