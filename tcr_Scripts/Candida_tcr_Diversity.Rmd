...
so_vdj <- readRDS(file = paste0(data_dir, "/so_vdj.rds"))
...
Next, I input the gini calculation formula. 
...
for (mouse in mice) {
 ...
 for (cluster in clusters) {
      # Subset by cluster
      subset <- IGT27_mouse[which(IGT27_mouse$CD_annot == cluster),]
      subset_calc <- calc_gini_mouse[which(calc_gini_mouse$CD_annot == cluster),]
      ...
      gini_indices_mouse_clonotype[mouse, cluster] <- gini_clonotype
      gini_indices_mouse_beta[mouse, cluster] <- gini_beta
      
      gini_indices_mouse_cdr3[mouse, cluster] <- gini_cdr3
      gini_indices_mouse_gearty[mouse, cluster] <- gini_gearty
...
ggplot(gf, aes(x = Mouse_New, y = value, color = Mouse_New)) +
  ...
  labs(x = "Cluster", y = "Gini Index Value", title = "Dotplot of Gini Index Values for each Mouse by Organ within Clusters", fill = "Organ") + 
    facet_wrap(~ cluster, scales = "free_x", nrow = 1) + # If I want to use this, change color = organ and rid of stuff in geom_point
    theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels by 45 degrees

values_cluster <- table(gf$value, gf$cluster)

gf_new <- gf[gf$cluster %in% colnames(values_cluster)[which(values_cluster["0",] < 3)],]

```

# Figure 15 Final
```{r}
fig_15 <- ggplot(gf_new, aes(x = Mouse_New, y = value, color = Mouse_New)) +
  geom_point(stat = "identity", size = 1.5) +
  #((aes(shape = organ, size = 1.5)))
  labs(x = "Cluster", y = "Gini Index Value", title = "Dotplot of Gini Index Values for each Mouse by Organ within Clusters", fill = "Organ", color = "Mice") + 
    facet_wrap(~ cluster, scales = "free_x", nrow = 1, labeller = label_wrap_gen(width=8)) + # If I want to use this, change color = organ and rid of stuff in geom_point
...
ggsave(filename = paste0(fig_directory, "figure_15.pdf"), plot = fig_15, device = "pdf", height = 6, width = 14)

ggsave(filename = paste0(fig_directory, "figure_15.jpeg"), plot = fig_15, device = "jpeg", height = 6, width = 12)
```
