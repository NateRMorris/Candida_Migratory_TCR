...
calc_gini <- so_vdj@meta.data[,c('cell_barcode', 'cdr3', 'clono.id_new', 'Mouse', 'Organ', 'RNA_clusters')]
...
for (mouse in mice) {
  ...
  for (organ in organs) {
    ...
    for (cluster in clusters) {
      # Subset by cluster
      subset <- IGT27_organ[which(IGT27_organ$RNA_clusters == cluster),]
      subset_calc <- calc_gini_organ[which(calc_gini_organ$RNA_clusters == cluster),]
...
for (mouse in mice) {
  ...
  for (organ in organs) {
    ...
    for (cluster in clusters) {
      # Subset by cluster
      subset <- IGT27_organ_downsample[which(IGT27_organ_downsample$RNA_clusters == cluster),]
      subset_calc <- calc_gini_organ_downsample[which(calc_gini_organ_downsample$RNA_clusters == cluster),]
