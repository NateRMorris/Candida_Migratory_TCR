...
seurat_object@meta.data[["TCR"]] <- seurat_object@meta.data[["cell_barcode"]] %in% IGT27_TCR$Barcode

seurat_object$clonotype_frequency <- IGT27_TCR[match(seurat_object@meta.data[["cell_barcode"]], IGT27_TCR$Barcode),]$Clonotype_frequency

seurat_object$clonotype_frequency[is.na(seurat_object$clonotype_frequency)] <- 0
```

```{r}
seurat_object$clonotype <- IGT27_TCR[match(seurat_object@meta.data[["cell_barcode"]], IGT27_TCR$Barcode),]$clonotype

table(rev(sort(table(seurat_object$clonotype))))

Idents(object = seurat_object) <- "clonotype"

seurat_object$clonotype_frequency <- IGT27_TCR[match(seurat_object@meta.data[["cell_barcode"]], IGT27_TCR$Barcode),]$Clonotype_frequency

table(rev(sort(table(seurat_object$clonotype))))

seurat_object@meta.data[["paired_across_organ"]] <- seurat_object@meta.data[["cell_barcode"]] %in% IGT27_TCR$Barcode[IGT27_TCR$paired_across_organ == TRUE]

Idents(object = seurat_object) <- "paired_across_organ"

seurat_object@meta.data[["paired_across_organ"]]<- as.factor(seurat_object$paired_across_organ)

seurat_object@meta.data[["duplicate"]] <- seurat_object@meta.data[["cell_barcode"]] %in% IGT27_TCR$Barcode[IGT27_TCR$duplicate == TRUE]

Idents(object = seurat_object) <- "duplicate"

seurat_object@meta.data[["duplicate"]]<- as.factor(seurat_object$duplicate)
```

```{r}
DimPlot(seurat_object, reduction = "umap_rna", group.by = "TCR") + ggtitle("TCR Seq by RNA UMAP") + NoLegend()

DimPlot(seurat_object, reduction = "umap_rna", group.by = "CD_annot", label = TRUE, label.size = 1.5, label.box = TRUE, repel = TRUE) + NoLegend()

DimPlot(seurat_object, reduction = "umap_both_", group.by = "CD_annot", label = TRUE, label.size = 1.5, label.box = TRUE, repel = TRUE) + NoLegend()
```


```{r}
DimPlot(seurat_object, reduction = "umap_rna", group.by = "TCR") + ggtitle("TCR Seq by RNA UMAP")
DimPlot(seurat_object, reduction = "umap_rna", group.by = "TCR", split.by = "Infection") + ggtitle("TCR Seq by RNA UMAP")
```


```
#setwd(data_dir)
saveRDS(seurat_object, file = paste0(data_dir, "seurat_new_12_08.rds"))
```


# Adding for Only TCR samples
```{r}
Idents(object = seurat_object) <- "TCR"

seurat_object.TCR <- subset(x = seurat_object, subset = (TCR == TRUE))
...
## 4.C-2 Clonotype Freq Group by Mouse

For here, if we choose to use our seurat_object.TCR, then we get the % of all TCR clonotypes that have n = 1, n = 2, and n > 3 proliferation.

However, if we use the seurat_object_prefiltered, then we get the % of all cells that have n = 0, n = 1, n = 2 and n > 3 proliferation.

```{r}
TCR_clonotype_freq <- as.data.frame(table(seurat_object$clonotype_frequency, seurat_object$Mouse_New))
...
TCR_clonotype_freq <- TCR_clonotype_freq %>%
    group_by(Mouse) %>%
    summarise(
        "No TCR" = sum(Freq[Clonotype_Freq == 0]) / sum(Freq) * 100,
        "n = 1" = sum(Freq[Clonotype_Freq == 1]) / sum(Freq) * 100,
        "n = 2" = sum(Freq[Clonotype_Freq == 2]) / sum(Freq) * 100,
        "n > 3" = sum(Freq[!Clonotype_Freq %in% c("0", "1", "2")]) / sum(Freq) * 100
    ) %>%
    ungroup()
...
level_order <- c('No TCR', 'n = 1', 'n = 2', 'n > 3') 

TCR_clonotype_freq_percentages_long <- TCR_clonotype_freq_percentages_long[TCR_clonotype_freq_percentages_long$Clonotype != "No TCR",]

# Plot the faceted barplot
prolif_perc <- ggplot(TCR_clonotype_freq_percentages_long, aes(factor(Clonotype, level = level_order), y = Percentage, fill = Clonotype)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = substr(as.character(Percentage), 0, 4)), position = position_stack(vjust = 0.5), angle = 45, size = 6) +
  labs(title = "Frequency of Clonotype Sizes by Mouse",
       x = "Size of Clonotype",
       y = "Percentage of all Cells",
       fill = "Number of Cells with identical Clonotype") +
  theme(legend.position = "bottom", axis.text.x = element_text(angle = 45, hjust=1)) + facet_wrap( ~ Mouse)
```

## 4.C-3 Clonotype Freq Group by Cluster

```{r}
TCR_clonotype_freq <- as.data.frame(table(seurat_object$clonotype_frequency, seurat_object$CD_annot))
colnames(TCR_clonotype_freq)[1] <- "Clonotype_Freq"
colnames(TCR_clonotype_freq)[2] <- "Cluster"
...
# Plot the stacked barplot
ggplot(TCR_clonotype_freq_percentages_long, aes(x = Cluster, y = Percentage, fill = Clonotype)) +
  geom_bar(stat = "identity", position = position_stack(reverse = TRUE)) +
  geom_text(aes(label = substr(as.character(Percentage), 0, 4), group = Clonotype), position = position_stack(vjust = 0.5, reverse = TRUE), size = 3) +
  labs(title = "Stacked Barplot of Frequency Percentages by Cluster",
       x = "Cluster",
       y = "Percentage",
       fill = "Clone Size") +
  scale_fill_manual(values = c("n = 1" = "blue", "n = 2" = "green", "n > 3" = "red")) +
...
# Plot the faceted barplot
ggplot(TCR_clonotype_freq_percentages_long, aes(factor(Clonotype, level = level_order), y = Percentage, fill = Clonotype)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = substr(as.character(Percentage), 0, 4)), position = position_stack(vjust = 1.2), angle = 45, size = 3.5) +
  ylim(-5,110) +
  labs(title = "Stacked Barplot of Frequency Percentages by Cluster",
       x = "Cluster",
       y = "Percentage",
       fill = "Number of Cells with identical Clonotype") +
  theme(legend.position = "right", axis.text.x = element_text(angle = 45, hjust=1)) + facet_wrap( ~ Cluster)
```
...
DimPlot(seurat_object.TCR[,seurat_object.TCR$clonotype_frequency > 1], reduction = "umap_rna", group.by = "CD_annot", split.by = "Infection", pt.size = (seurat_object.TCR[,seurat_object.TCR$clonotype_frequency > 1]$clonotype_frequency)*0.6, shape.by = "paired_across_organ") + ggtitle("Proliferated Clonotypes - Color by Cluster, Size by # of Cells with same Clonotype, Shape by Migratory") + theme(plot.title = element_text(size = 7, face = "bold"))

DimPlot(seurat_object.TCR[,seurat_object.TCR$clonotype_frequency > 2], reduction = "umap_rna", group.by = "CD_annot", split.by = "Infection", pt.size = (seurat_object.TCR[,seurat_object.TCR$clonotype_frequency > 2]$clonotype_frequency)*0.6, shape.by = "paired_across_organ") + ggtitle("Proliferated Clonotypes - Color by Cluster, Size by # of Cells with same Clonotype, Shape by Migratory") + theme(plot.title = element_text(size = 7, face = "bold"))
...
```{r}
# Showing all Cells
DimPlot(seurat_object.TCR, reduction = "umap_rna", group.by = "CD_annot", split.by = "Infection", cells.highlight = which(seurat_object.TCR$duplicate == TRUE), pt.size = (seurat_object.TCR$clonotype_frequency)*0.6) + ggtitle("All cells - Proliferating Highlighted") + theme(plot.title = element_text(size = 7, face = "bold")) + NoLegend()

DimPlot(seurat_object.TCR, reduction = "umap_rna", group.by = "CD_annot", split.by = "Infection", cells.highlight = which(seurat_object.TCR$paired_across_organ == TRUE), pt.size = (seurat_object.TCR$clonotype_frequency)*0.6) + ggtitle("All cells - Paired Highlighted") + theme(plot.title = element_text(size = 7, face = "bold")) + NoLegend()

DimPlot(seurat_object.TCR, reduction = "umap_rna", group.by = "CD_annot", split.by = "Infection", pt.size = (seurat_object.TCR$clonotype_frequency)*0.6, shape.by = "paired_across_organ") + ggtitle("All cells - Color by Cluster, Size by # of Cells with same Clonotype, Shape by if across Organ") + theme(plot.title = element_text(size = 7, face = "bold"))
```


## 4.D Migratory and Duplicate Frequency Distributions

### Migratory Clonotypes Frequency Distribution
...
#TCR_paired_by_RNA <- as.data.frame(table(seurat_object.TCR$CD_annot[seurat_object.TCR$paired_across_organ == TRUE]))
TCR_paired_by_RNA <- as.data.frame(table(seurat_object.TCR$CD_annot[seurat_object.TCR$paired_across_organ == TRUE],seurat_object.TCR$Mouse_New[seurat_object.TCR$paired_across_organ == TRUE]))
...
#TCR_duplicate_by_RNA <- as.data.frame(table(seurat_object.TCR$CD_annot[seurat_object.TCR$duplicate == TRUE]))
TCR_duplicate_by_RNA <- as.data.frame(table(seurat_object.TCR$CD_annot[seurat_object.TCR$duplicate == TRUE],seurat_object.TCR$Mouse_New[seurat_object.TCR$duplicate == TRUE]))
...
clusters <- sort(unique(seurat_object.TCR$CD_annot))
...
for (organ in organs){
  for (cluster in clusters){
    ...
    success_total <- as.data.frame(table(seurat_object.TCR$Infection[seurat_object.TCR$CD_annot == cluster & seurat_object.TCR$Organ == organ]))
    ...
    denom_total <- as.data.frame(table(seurat_object$Infection[seurat_object$CD_annot == cluster & seurat_object$Organ == organ]))
    ...
    # Unique Clonotypes
    success_clono <- as.data.frame(table(seurat_object.TCR$clonotype[seurat_object.TCR$CD_annot == cluster & seurat_object.TCR$Organ == organ], seurat_object.TCR$Infection[seurat_object.TCR$CD_annot == cluster & seurat_object.TCR$Organ == organ]))
    ...
    # Proliferating TCR
    success_prolif <- as.data.frame(table(seurat_object.TCR$Infection[seurat_object.TCR$CD_annot == cluster & seurat_object.TCR$Organ == organ & seurat_object.TCR$duplicate == TRUE]))
    ...
    # Migratory TCR
    success_migratory <- as.data.frame(table(seurat_object.TCR$Infection[seurat_object.TCR$CD_annot == cluster & seurat_object.TCR$Organ == organ & seurat_object.TCR$paired_across_organ == TRUE]))
...
existing_metadata <- so_vdj$CD_annot
...
data_melted_mouse$Percentage <- (data_melted_mouse$Frequency)*100

ggplot(data_melted_mouse, aes(x = reorder(Row, -Percentage, sum), y = Percentage, fill = Sample)) +
  geom_bar(stat = "identity") +
  labs(
    title = "Stacked Barplot of Clonotype Frequencies by Mouse",
    x = "Clonotype",
    y = "% Clonotype Make up of Each Mouse's Cells") +
...
ggplot(data_melted_mouse, aes(x = reorder(Row, -Frequency, sum), y = Percentage, fill = Sample)) +
  geom_bar(stat = "identity") +
  labs(
    title = "Stacked Barplot of Clonotype Frequencies by Mouse",
    x = "Clonotype",
    y = "% of Each Mouses Cells with Each Clonotype",
    fill = "Mouse") +
  theme_minimal() +
  theme(legend.position = "top", axis.text.x = element_text(angle = 45, hjust = 1)) + coord_flip()
...
get_frequency_table_Cluster <- function(clonotype) {
  table(so_vdj$CD_annot[so_vdj$clonotype_id == clonotype])
}
...
for (i in sort(unique(so_vdj$CD_annot))){
  normalize_df_by_Cluster[i,] <- normalize_df_by_Cluster[i,] / (length(so_vdj$cell_barcode[so_vdj$CD_annot == i]))
}
```
...
# Top 20 for Each Mouse by Cluster
# Fig 14

```{r}
get_frequency_table_mouse_Cluster <- function(mouse, clonotype) {
  so_temp <- so_vdj[,so_vdj$Mouse_New == mouse]
  table(so_temp$CD_annot[so_temp$clonotype_id == clonotype])
}

freq_df_list <- list()

for (mouse in unique(so_vdj$Mouse_New)){
  #temp_df <- t(as.data.frame(rev(sort(table(so_vdj$clonotype_id[so_vdj$Mouse_New == mouse])))[1:20]))
  temp_df <- t(as.data.frame(rev(sort(table(so_vdj$clonotype_id[so_vdj$Mouse_New == mouse])))[1:50])) # 50 contains all proliferating for every mouse
  colnames(temp_df) <- temp_df["Var1",]
  temp_df <- temp_df[,as.numeric(temp_df["Freq",]) > 1]
  
  freq_df_temp <- as.data.frame(sapply(colnames(temp_df), function(col) get_frequency_table_mouse_Cluster(mouse, col)))
  
  freq_df_temp <- freq_df_temp[rowSums(freq_df_temp != 0) > 0, ]
  
  freq_df_melted <- as.data.frame(t(freq_df_temp)) %>%
  rownames_to_column(var = "Row") %>%
  pivot_longer(cols = -Row, names_to = "Cluster", values_to = "Num_Cells")
  
  freq_df_melted$Frequency <- (freq_df_melted$Num_Cells / length(so_vdj$clonotype_id[so_vdj$Mouse_New == mouse]))
  freq_df_melted$Percent <- freq_df_melted$Frequency * 100
  
  freq_df_melted$Mouse_New <- mouse
  
  freq_df_list[[mouse]] <- freq_df_melted
}

all_data <- rbind(freq_df_list$`No Infection Male`, freq_df_list$`Infected Female`, freq_df_list$`Infected Male` )

all_clonos_2_up <- ggplot(all_data, aes(x = reorder(Row, -Frequency, sum), y = Percent, fill = Cluster)) +
    geom_bar(stat = "identity") +
    labs(title = "Frequency of Proliferating Clonotypes per Mouse",
         x = "Clonotype",
         y = "Percent of Mouse' Cells",
         fill = "Cluster") +
    theme_minimal() +  facet_wrap(vars(Mouse_New), scales = "free_y") +
    theme(legend.position = "bottom", axis.text.x = element_text(angle = 45, hjust = 1), strip.text = element_text(face = "bold", size = 12)) + coord_flip()
...
```{r}
weighted_sum_row <- function(row) {
  sum(as.numeric(names(row)) * row)
}


no_male <- table(all_data$Cluster[all_data$Mouse_New == "No Infection Male"], all_data$Num_Cells[all_data$Mouse_New == "No Infection Male"])
colnames(no_male) <- as.numeric(colnames(no_male))
weighted_sums_no_male <- apply(no_male, 1, weighted_sum_row)

ca_male <- table(all_data$Cluster[all_data$Mouse_New == "Infected Male"], all_data$Num_Cells[all_data$Mouse_New == "Infected Male"])
colnames(ca_male) <- as.numeric(colnames(ca_male))
weighted_sums_ca_male <- apply(ca_male, 1, weighted_sum_row)

ca_female <- table(all_data$Cluster[all_data$Mouse_New == "Infected Female"], all_data$Num_Cells[all_data$Mouse_New == "Infected Female"])
colnames(ca_female) <- as.numeric(colnames(ca_female))
weighted_sums_ca_female <- apply(ca_female, 1, weighted_sum_row)

df <- as.data.frame(cbind(weighted_sums_ca_female, weighted_sums_ca_male, weighted_sums_no_male))
df$Cluster <- rownames(df)
```


```{r}
# Calculate the total number of TCRs for each mouse
total_TCRs_ca_female <- sum(df$weighted_sums_ca_female)
total_TCRs_ca_male <- sum(df$weighted_sums_ca_male)
total_TCRs_no_male <- sum(df$weighted_sums_no_male)

# Calculate the column sums for each mouse
df$Ca_female_tcr_clust_perc <- (df$weighted_sums_ca_female) / total_TCRs_ca_female
df$Ca_male_tcr_clust_perc <- (df$weighted_sums_ca_male) / total_TCRs_ca_male
df$No_male_tcr_clust_perc <- (df$weighted_sums_no_male) / total_TCRs_no_male
```

```{r}
df_perc <- df[,4:7]
df_melt_perc <- melt(df_perc)
# Create the bar plot using ggplot2
df_melt_perc$Cluster <- factor(df_melt_perc$Cluster, levels = unique(df_melt_perc$Cluster))

df_melt_perc <- df_melt_perc %>%
  mutate(Mouse_Type = if_else(variable == "Ca_male_tcr_clust_perc", "Infected Male",
                              if_else(variable == "No_male_tcr_clust_perc", "No Infection Male",
                                      if_else(variable == "Ca_female_tcr_clust_perc", "Infected Female", variable, variable),
                                      variable),
                              variable))
df_melt_perc$percentage <- df_melt_perc$value*100
# Create the bar plot with facets
prolif_clust <- ggplot(df_melt_perc, aes(x = Cluster, y = percentage, fill = Mouse_Type)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Percentage of Mouse's Proliferating Cells in Each Cell Type",
       x = "Cell Type",
       y = "Percentage",
       fill = "Mouse") +
  scale_fill_manual(values = c("darkred", "navyblue", "forestgreen")) +
  facet_wrap(~Cluster, scales = "free_x", nrow = 1, labeller = label_wrap_gen(width=10)) + theme(axis.text.x=element_blank())  # Facet by cell type
```


```{r}
prolif <- cowplot::plot_grid(all_clonos_2_up, prolif_perc, ncol = 2, labels = c("A", "B"), rel_widths = c(1, 0.4))
fig_14 <- cowplot::plot_grid(prolif, prolif_clust, ncol = 1, labels = c("", "C"))
...
ggsave(filename = paste0(fig_directory, "figure_14.pdf"), plot = fig_14, device = "pdf", height = 16, width = 24)

ggsave(filename = paste0(fig_directory, "figure_14.jpeg"), plot = fig_14, device = "jpeg", height = 16, width = 24)
```
...
#A
fig14_a <- cowplot::plot_grid(all_clonos_2_up, labels = c("A"), label_size = 30)

ggsave(filename = paste0(fig_directory, "figure_14a.pdf"), plot = fig14_a, device = "pdf", height = 10, width = 14)

ggsave(filename = paste0(fig_directory, "figure_14a.jpeg"), plot = fig14_a, device = "jpeg", height = 10, width = 14)


#B

fig14_b <- cowplot::plot_grid(prolif_perc, labels = c("B"), label_size = 30)

ggsave(filename = paste0(fig_directory, "figure_14b.pdf"), plot = fig14_b, device = "pdf", height = 8, width = 8)

ggsave(filename = paste0(fig_directory, "figure_14b.jpeg"), plot = fig14_b, device = "jpeg", height = 8, width = 8)


#C

fig14_c <- cowplot::plot_grid(prolif_clust, labels = c("C"), label_size = 30)

ggsave(filename = paste0(fig_directory, "figure_14c.pdf"), plot = fig14_c, device = "pdf", height = 8, width = 18)

ggsave(filename = paste0(fig_directory, "figure_14c.jpeg"), plot = fig14_c, device = "jpeg", height = 8, width = 18)

```

# Number of Proliferative Clonotypes Statistic

```{r}
#Infected
prolif_inf <- (sum(table(so_vdj$clonotype_id[so_vdj$Mouse_New == "Infected Male"]) > 1) + sum(table(so_vdj$clonotype_id[so_vdj$Mouse_New == "Infected Female"]) > 1))
num_clono_inf <- length(unique(so_vdj$clonotype_id[so_vdj$Mouse_New == "Infected Male"])) + length(unique(so_vdj$clonotype_id[so_vdj$Mouse_New == "Infected Female"]))

# Non-infected
prolif_no <- sum(table(so_vdj$clonotype_id[so_vdj$Mouse_New == "No Infection Male"]) > 1)
num_clono_no <- length(unique(so_vdj$clonotype_id[so_vdj$Mouse_New == "No Infection Male"]))

result <- prop.test(c(prolif_inf, prolif_no), c(num_clono_inf, num_clono_no))

# Print the result
print(result)

# Percent of clonotypes that are proliferative in Infection
(prolif_inf / num_clono_inf)*100

# Percent of clonotypes that are proliferative in Non-infection
(prolif_no / num_clono_no)*100
```

# Number of SMALL Proliferative Clonotypes Statistic

```{r}
#Infected
small_prolif_inf <- (sum(table(so_vdj$clonotype_id[so_vdj$Mouse_New == "Infected Male"]) == 2) + sum(table(so_vdj$clonotype_id[so_vdj$Mouse_New == "Infected Female"]) == 2))

# Non-infected
small_prolif_no <- sum(table(so_vdj$clonotype_id[so_vdj$Mouse_New == "No Infection Male"]) == 2)

result_small <- prop.test(c(small_prolif_inf, small_prolif_no), c(num_clono_inf, num_clono_no))
result_small

# Percent of clonotypes that are small proliferative in Infection
(small_prolif_inf / num_clono_inf)*100

# Percent of clonotypes that are proliferative in Non-infection
(small_prolif_no/ num_clono_no)*100
```
...
```{r}
clusters <- sort(unique(so_vdj$CD_annot))
cluster_D50 <- list()
...
for (cluster in clusters) {
  #print(cluster)
  clonotype_ids <- so_vdj[, so_vdj$CD_annot == cluster]@meta.data[["clonotype_id"]]
...
ggplot(cluster_simp_df[cluster_simp_df$Cluster != "cluster_16",], aes(x = Cluster, y = simp_Score, color = Cluster)) +
  geom_point(size = 3) +
  labs(
...
#rownames(cluster_gini_df) <- paste0("cluster_", rownames(cluster_gini_df))
cluster_gini_df$Cluster <-  paste0("cluster_", rownames(cluster_gini_df))
...
clono_ids <- function(x, y){
    tryCatch(
        {
        clono_ids <- temp_seurat$clonotype_id[temp_seurat$Mouse_New == x & temp_seurat$CD_annot == y]
        return(clono_ids)
        },
        error=function(e) {
...
#RNA_clusts <- sort(unique(temp_seurat$CD4_clust_new11))
RNA_clusts <- sort(unique(temp_seurat$CD_annot))
...
#df_long <- df_long_gini[df_long_gs$Cluster != "cluster_16",]
#df_long <- df_long_gs[df_long_gs$Cluster != "cluster_16",]
df_long <- df_long_gini

ggplot(df_long, aes(x = Cluster, y = Value, color = sample, shape = Infection)) +
  geom_point() +
  labs(x = "Cluster", y = "Simpson Index Value") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle("Simpson Diversity Index by Cluster and Sample")
...
so_vdj[which(so_vdj$CD_annot == 5),] |>
  plot_diversity(
    data_col    = "clonotype_id",
    cluster_col = "temp",
    group_col = "Mouse",
...
```{r}
saveRDS(so_vdj, file = paste0(data_dir,"so_vdj.rds"))
...
SaveH5Seurat(so_vdj, filename = paste0(data_dir,"so_vdj.h5Seurat"))
...
```{r}
df <- as.data.frame(table(cdr3_clustering$RNA_new, cdr3_clustering$cdr3_cluster_0.5))
colnames(df) <- c("RNA_new", "CDR3_Clust", "Freq")
...
ggplot(percentage_data, aes(x = RNA_new, y = percentage, fill = CDR3_Clust)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(x = "CD_annot", y = "Percentage") +
  scale_fill_discrete(name = "CDR3_Clust") +
  theme_minimal()
```


```{r}
df <- as.data.frame(table(cdr3_clustering$Infection, cdr3_clustering$cdr3_cluster_0.5))
colnames(df) <- c("sample_name", "CDR3_Clust", "Freq")
...
my_comparisons <- list(c("No", "Ca"))

# Create a ggplot barplot
ggplot(percentage_data, aes(x = sample_name, y = percentage, fill = CDR3_Clust)) +
  geom_bar(stat = "identity") +
  labs(x = "Condition", y = "Percentage of Condition Cells in Each CDR3 Clust") +
  scale_fill_discrete(name = "Sample") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + facet_wrap(~CDR3_Clust) +
  stat_compare_means(comparisons = my_comparisons, method = "t.test", label =  "p.signif", paired = FALSE)
```

```{r}
df <- as.data.frame(table(cdr3_clustering$Mouse_New, cdr3_clustering$cdr3_cluster_0.5))
colnames(df) <- c("Mouse", "CDR3_Clust", "Freq")
...
percentage_data$infection <- substr(percentage_data$Mouse,0,3)

my_comparisons <- list(c("No", "Ca"))

# Create a ggplot barplot
ggplot(percentage_data, aes(x = Mouse, y = percentage, fill = infection)) +
  geom_bar(stat = "identity") +
  labs(x = "Condition", y = "Percentage of Condition Cells in Each CDR3 Clust") +
  scale_fill_discrete(name = "Sample") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + facet_wrap(~CDR3_Clust)
...
```{r}
df <- as.data.frame(table(cdr3_clustering$sample_name, cdr3_clustering$cdr3_cluster_0.5))
colnames(df) <- c("sample_name", "CDR3_Clust", "Freq")
...
percentage_data$organ <- substr(percentage_data$sample_name,4,7)
percentage_data$infection <- substr(percentage_data$sample_name,0,2)

# Create a ggplot barplot
ggplot(percentage_data, aes(x = sample_name, y = percentage, fill = substr(sample_name,0,2))) +
  geom_bar(stat = "identity") +
  labs(x = "Sample", y = "Percentage of Sample's Cells in CDR3 Clust") +
  scale_fill_discrete(name = "Condition") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + facet_wrap(~CDR3_Clust+organ) 
...
ggplot(gini, aes(x = mouse, y = V1, color = mouse)) +
  geom_boxplot() +
  labs(
    x = "Condition",
    y = "Gini Index Value",
    title = "Boxplot of Frequency by Cluster and Condition") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + facet_grid(. ~ clust) 


gini_new <- gini[gini$clust %in% c("Effector CD4- Lung", "Th17 CD4", "Treg/Th2 CD4- Colon/Lung", "Tfh CD4- mLns", "KLRG1+ Eff CD8- Lung","Effector CD4- Lung/Colon", "TCRGD+ CD8- Lung", "Ly6c2+ CD8- Lung/Colon", "Naive CD4- Lung"),]
# Removing clusters with 0 gini index
ggplot(gini_new, aes(x = mouse, y = V1, color = mouse)) +
  geom_boxplot() +
  labs(
    x = "Condition",
    y = "Gini Index Value",
...
for (sample in unique(so_vdj$sample_name)){
  ...
  tcrb_list_clust5[[sample]] <- as.list(gsub(".*?;", "", so_vdj$cdr3[which(so_vdj$chains == "TRA;TRB" & so_vdj$sample_name == sample & so_vdj$CD_annot == 5)]))
}
...
There is nothing significantly different between paired samples with and without infection
```
Idents(object = seurat_object.TCR) <- "paired_across_organ"

seurat_object.TCR.paired <- subset(x = seurat_object.TCR, subset = (paired_across_organ == TRUE))

Idents(seurat_object.TCR.paired) <- "Infection"
DefaultAssay(seurat_object.TCR.paired) <- "RNA"
paired_ca_markers <- FindMarkers(seurat_object.TCR.paired, ident.1 = "Ca", only.pos = TRUE,  min.pct = 0.05,logfc.threshold = 0.05)
paired_ca_markers$entrezid <- mapIds(org.Mm.eg.db, keys = rownames(paired_ca_markers), column = "ENTREZID", keytype = "SYMBOL") # Could also use ENSEMBL instead of ENTREZ
paired_ca_go_terms_bp <- enrichGO(paired_ca_markers$entrezid, "org.Mm.eg.db", keyType = "ENTREZID", ont = "BP", pvalueCutoff = 0.05, pAdjustMethod = "BH")
dotplot(paired_ca_go_terms_bp, showCategory=10) + ggtitle("Duplicate infection")
```


# GO Enrichment of Duplicate

There is nothing significantly different between duplicate samples with and without infection
```
Idents(object = seurat_object.TCR) <- "duplicate"
...
# 8. Gene/Protein Expression in Clusters of Interest Inf vs Noninf
...
https://www.nature.com/articles/s41598-017-01411-1 - Immune-checkpoint protein VISTA critically regulates the IL-23/IL-17 inflammatory axis 
