...
```{r}
DefaultAssay(seurat_object_prefiltered) <- "RNA"

seurat_object_prefiltered@meta.data[["TCR"]] <- seurat_object_prefiltered@meta.data[["cell_barcode"]] %in% IGT27_TCR$Barcode

seurat_object_prefiltered$clonotype_frequency <- IGT27_TCR[match(seurat_object_prefiltered@meta.data[["cell_barcode"]], IGT27_TCR$Barcode),]$Clonotype_frequency

seurat_object_prefiltered$clonotype_frequency[is.na(seurat_object_prefiltered$clonotype_frequency)] <- 0


```


## Creating New Seurat Object with all cells and TCR information
```{r}
seurat_object_new <- seurat_object_prefiltered %>% NormalizeData(verbose = FALSE) %>% FindVariableFeatures(selection.method = "vst", nfeatures = 4000) %>% ScaleData(verbose = TRUE)
seurat_object_new_new <- seurat_object_new %>% RunPCA(pc.genes = t_cells@var.genes, npcs = 30, verbose = FALSE) %>% RunUMAP(dims = 1:25) %>% RunTSNE(dims = 1:25) %>% FindNeighbors(dims = 1:25,compute.SNN=TRUE) %>% FindClusters(resolution = c(0.1,0.2,0.4,0.8,1)) %>% DoopenTSNE(reduction_save = "openTSNE",dims_use = 1:25) 


```

```{r}
seurat_object_new_new$clonotype <- IGT27_TCR[match(seurat_object_new_new@meta.data[["cell_barcode"]], IGT27_TCR$Barcode),]$clonotype

table(rev(sort(table(seurat_object_new_new$clonotype))))

Idents(object = seurat_object_new_new) <- "clonotype"

seurat_object_new_new$clonotype_frequency <- IGT27_TCR[match(seurat_object_new_new@meta.data[["cell_barcode"]], IGT27_TCR$Barcode),]$Clonotype_frequency

table(rev(sort(table(seurat_object_new_new$clonotype))))

seurat_object_new_new@meta.data[["paired_across_organ"]] <- seurat_object_new_new@meta.data[["cell_barcode"]] %in% IGT27_TCR$Barcode[IGT27_TCR$paired_across_organ == TRUE]

Idents(object = seurat_object.TCR) <- "paired_across_organ"

seurat_object_new_new@meta.data[["paired_across_organ"]]<- as.factor(seurat_object_new_new$paired_across_organ)

seurat_object_new_new@meta.data[["duplicate"]] <- seurat_object_new_new@meta.data[["cell_barcode"]] %in% IGT27_TCR$Barcode[IGT27_TCR$duplicate == TRUE]

Idents(object = seurat_object_new_new) <- "duplicate"

seurat_object_new_new@meta.data[["duplicate"]]<- as.factor(seurat_object_new_new$duplicate)
```

```{r}
#DimPlot(seurat_object_new_new, reduction = "umap_rna", group.by = "TCR") + ggtitle("TCR Seq by RNA UMAP")

DimPlot(seurat_object_new_new, reduction = "openTSNE", group.by = "RNA_clusters", label = TRUE, split.by = "Infection")
```


```{r}
DimPlot(seurat_object_new_new, reduction = "umap_rna", group.by = "TCR") + ggtitle("TCR Seq by RNA UMAP")
DimPlot(seurat_object_new_new, reduction = "umap_rna", group.by = "TCR", split.by = "Infection") + ggtitle("TCR Seq by RNA UMAP")

DimPlot(seurat_object_new_new, reduction = "openTSNE", group.by = "TCR") + ggtitle("TCR Seq by RNA UMAP")
DimPlot(seurat_object_new_new, reduction = "openTSNE", group.by = "TCR", split.by = "Infection") + ggtitle("TCR Seq by RNA UMAP")
```


```{r}
#setwd(data_dir)
saveRDS(seurat_object_new_new, file = paste0(data_dir, "seurat_new_12_08.rds"))
```
...
```{r}
TCR_clonotype_freq <- as.data.frame(table(seurat_object_prefiltered$clonotype_frequency, seurat_object_prefiltered$Mouse_New))
...
ggplot(TCR_clonotype_freq_percentages_long, aes(factor(Clonotype, level = level_order), y = Percentage, fill = Clonotype)) +
  ...
  labs(title = "Stacked Barplot of Frequency Percentages by Mouse",
       x = "Size of Clonotype for each Cell",
       y = "Percentage of all Cells",
       fill = "Number of Cells with identical Clonotype") +
  theme(legend.position = "right", axis.text.x = element_text(angle = 45, hjust=1)) + facet_wrap( ~ Mouse)
```

## 4.C-3 Clonotype Freq Group by Cluster

```{r}
TCR_clonotype_freq <- as.data.frame(table(seurat_object_prefiltered$clonotype_frequency, seurat_object_prefiltered$RNA_clusters))
colnames(TCR_clonotype_freq)[1] <- "Clonotype_Freq"
...
ggplot(percentage_data, aes(x = RNA_new, y = percentage, fill = CDR3_Clust)) +
  ...
  labs(x = "RNA_clusters", y = "Percentage") +
