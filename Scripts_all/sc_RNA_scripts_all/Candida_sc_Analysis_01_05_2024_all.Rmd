...
# 2.D ## Identifying Th17 cells

```{r}
seurat_object <- readRDS(file = paste0(data_dir, "seurat_object_high_res.rds"))
```

```{r}
DimPlot(seurat_object, group.by = "RNA_high")
```

Shows that Th17 expression cells are higher in Ca
```{r}
...
seurat_object$clust_inf <- paste0(seurat_object$RNA_highres_clust,"_", seurat_object$Infection)

AverageExpression(seurat_object, assay = "RNA", features = c("Il17a", "Rora"), group.by = c("RNA_highres_clust", "Infection"))

Th17 <- c("Rora","Il26","Il17a","Il17f","Ccr6", "Il23r", "Il22")

Stacked_VlnPlot(seurat_object, features = Th17, group.by = "RNA_highres_clust")

Stacked_VlnPlot(seurat_object, features = Th17, group.by = "clust_inf") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
```
...
```{r}
DotPlot(seurat_object, assay = "RNA", features = Th17, group.by = "RNA_highres_clust", split.by = "Infection") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + coord_flip()

VlnPlot(seurat_object, assay = "RNA", features = c("Il17a"), group.by = "RNA_highres_clust", split.by = "Infection") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
## Cluster 17 - RORalpha very high

https://pubmed.ncbi.nlm.nih.gov/18164222/

Cluster 17 (our old cluster 11) has high Ror alpha, but low levels of Il17a. It is known that Rorg helps T cell differentiation into Th-17 cells, however this paper states that Ror alpha also helps the differentiation into Th-17 cells.
...
#NOT IMPORTANT: Organ composition for each cluster
...
# 3. SUBSETTING ONLY CD4 and Differentiated T CELLS
...
seurat_object_CD4_diff <- seurat_object_CD4_diff %>% RunPCA(pc.genes = seurat_object_CD4_diff@var.genes, npcs = 30, verbose = FALSE) %>%  RunUMAP(dims = 1:25, reduction.name = "umap_cd4_") %>% RunTSNE(dims = 1:25, reduction.name = "tsne_cd4_") %>% FindNeighbors(dims = 1:25,compute.SNN=TRUE) %>% FindClusters(resolution = c(0.1,0.2,0.4,0.8,1)) %>% DoopenTSNE(reduction_save = "openTSNE_cd4_",dims_use = 1:25) %>% DoForceAtlas2(so.clust5, reduction_use = "pca", reduction_save = "fa2_cd4_", dims_use = 1:25)
```
...
FeaturePlot(object = seurat_object_CD4_diff, features = "Il17.gene.set.score") 
...
# MAYBE HE WANTS IT THIS WAY? I dont this is computationally correct to do though
...
FeaturePlot(seurat_object, features = c("CD44"),
            reduction = 'umap_rna', 
            cols = c("yellow","red"), ncol = 4) + NoLegend() + NoAxes()
