...
# Visualizing both CD4 and CD8 Numbered Clusters in Joined Seurat Object

```{r}
seurat_object_CD8$CD8_clust_star <- paste0(seurat_object_CD8$CD8_clust, "*")

CD8_clust_num <- as.data.frame(seurat_object_CD8$CD8_clust)
colnames(CD8_clust_num) <- c("CD8_clust_num")
CD8_clust_num$CD8_clust_num_new <- paste0(CD8_clust_num$CD8_clust_num, "*")

colnames(CD8_clust_num) <- c("clust_num_old", "clust_num")
CD8_clust_num$barcodes <- rownames(CD8_clust_num)

CD8_clust_num <- CD8_clust_num[, !(names(CD8_clust_num) == "clust_num_old")]

colnames(CD4_clust_num) <- c("clust_num")
CD4_clust_num$barcodes <- rownames(CD4_clust_num)

all_clust_num <- rbind(CD4_clust_num, CD8_clust_num)
all_clust_num$barcodes <- rownames(all_clust_num)

saveRDS(all_clust_num, file = paste0(data_dir, "/all_clust_numbers.rds"))
...
```{r}
DimPlot(seurat_object, reduction = "umap_both_", group.by = "clust_num", label = TRUE, repel = TRUE, label.box = TRUE)
DimPlot(seurat_object, reduction = "umap_both_", group.by = "clust_num", split.by = "Infection") + NoLegend()

DimPlot(seurat_object, reduction = "tsne_both_", group.by = "clust_num", label = TRUE, repel = TRUE, label.box = TRUE)
...
# 1.A-2 Fig 4 - Marker Proteins and Up DEGs per Cluster
...
feature_plots_list_rna <- lapply(features_cd4_rna, create_feature_plot)
...
```{r}
fig_8a_cd4 <- cowplot::plot_grid(cd4_vln, up_cd4, labels = c("C", "D"), ncol = 1)
```

# 1.A-3 DEGS INTER CLUSTER B/W CONDITION
...
# 1.B Top 5 Genes Upregulated for Cluster
...
# 1.C Top 5 Proteins for Cluster
...
# 1.D DotPlot for Cell Characterization
...
# Pseudobulk Heatmap by Top 5 Genes
...
# Full Heatmap for Characterization
...
# Figure 4 Part 2 - Violin, DotPlot CD4 Exp


```{r}
#fig_8a_cd4 <- cowplot::plot_grid(cd4_vln, up_cd4, labels = c("A", "B"), ncol = 1)
#cd4_dot_plot
fig_8b_cd4 <- cowplot::plot_grid(fig_8a_cd4, cd4_dot_plot, labels = c("", "E"), ncol = 2, rel_widths = c(0.7, 1))
```

# Supplemental Figure CD4 - Volcano and Heatmap

```{r}
supp_cd4 <- cowplot::plot_grid(cd4_heatmap, volcano_grid_plot_cd4, labels = c("A", "B"), ncol = 2, rel_widths = c(1, 1))
...
clusterProfiler::dotplot(clust_GO, showCategory=5, font.size = 8) + ggtitle("CD4 Cluster GO ORA")

treeplot(clust_GO, showCategory = 5)
...
dotplot(clust_KEGG, showCategory=3, font.size = 8) + ggtitle("CD4 Cluster KEGG ORA")
...
# Cluster 6 Dotplot for each organ
...
# 2.A-2 Fig 5 - Marker Proteins and Up DEGs per Cluster
...
# 2.A-3 DEGS INTER CLUSTER B/W CONDITION
...
# 2.B Top 5 Genes Upregulated for Cluster
...
# 2.C Top 5 Proteins for Cluster
...
# 2.D-2 - ADT MARKER/CHARACTERIZING PROTEINS
...
cd8_dot_plot <- DotPlot(
    ...
    features = cd8_adt_list, 
    scale = TRUE, 
    group.by = "CD8_clust_annot") + scale_colour_gradient2(low = "#ff0000", mid = "#f5f5f5", high = "#0000ff") + theme(axis.text.x = element_text(angle = 45, margin = margin(t = 21)))

Idents(seurat_object_CD8) <- "CD8_clust"
VlnPlot( seurat_object_CD8, 
    features = cd8_adt_list, stack = TRUE, flip = TRUE) + NoLegend()
...
# Figure 9  Part 2 - Violin, DotPlot CD8 Exp
...
```{r}
#fig_8a_cd8 <- cowplot::plot_grid(cd4_vln, up_cd4, labels = c("A", "B"), ncol = 1)
#cd8_dot_plot
fig_8b_cd8 <- cowplot::plot_grid(fig_8a_cd8, cd8_dot_plot, labels = c("", "E"), ncol = 2, rel_widths = c(1, 1))
```
...
clusterProfiler::dotplot(clust_GO, showCategory=5, font.size =8) + ggtitle("CD8 Cluster GO ORA")
...
```{r}
Neuron <- c("Calca", "Ramp1", "Ramp3", "Ntrk3", "Gch1", "Nr3c1", "Bex3")
Idents(seurat_object) <- "CD_annot"
VlnPlot(seurat_object, features = Neuron, group.by = "CD_annot", stack = TRUE, flip = TRUE, fill.by = "ident") + NoLegend()
...
cell_cycle_a <- RidgePlot(seurat_object_CD4, features = c("G2M.Score"), ncol = 1) + NoLegend() + labs(y = "CD4 Cluster")
cell_cycle_b <- RidgePlot(seurat_object_CD4, features = c("S.Score"), ncol = 1)+ NoLegend() + labs(y = "CD4 Cluster")
...
Idents(seurat_object_CD8) <- "CD8_clust"
cell_cycle_a <- RidgePlot(seurat_object_CD8, features = c("G2M.Score"), ncol = 1) + NoLegend() + labs(y = "CD8 Cluster")
cell_cycle_b <- RidgePlot(seurat_object_CD8, features = c("S.Score"), ncol = 1)+ NoLegend() + labs(y = "CD8 Cluster")
