...
All_genes <- list(Global_T,CD4,CD8,Th17,Tfh_Th1_Th2_Th17,Tregs,Naive,Memory,Eff_Mem,Activated,Exhausted, NK)
names(All_genes) <- c("Global_T","CD4","CD8","Th17","Tfh_Th1_Th2_Th17","Tregs","Naive","Memory","Eff_Mem","Activated","Exhausted", "NK")
...
```{r}
pathways.show <- c("IL16") # TGFb # IFN-II # IL-16
# Hierarchy plot
# Here we define `vertex.receive` so that the left portion of the hierarchy plot shows signaling to fibroblast and the right portion shows signaling to immune cells 
vertex.receiver = seq(1,4) # a numeric vector. 
netVisual_aggregate(cellchat, signaling = pathways.show,  vertex.receiver = vertex.receiver)
